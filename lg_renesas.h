...
int lgren_read (int device, char source, mmcdata_s * buff, size_t pos,
		size_t size);
int lgren_write (int device, char source, mmcdata_s * buff, size_t pos,
		 size_t size);
char get_firminfo_drive (int device, char *pattern, size_t patsize,
			 firminfo * fi);
char get_firminfo_buff (char *buff, char *pattern, size_t bsize,
			size_t patsize, firminfo * fi);
...
char firm_verify (int device, char *buff, size_t firm_start,
		  size_t firm_size);
size_t firm_dumper (int device, char **inbuff, int loc, size_t pos,
		    size_t dsize);
char firm_spliter (char *buff, size_t max_size, const char *outfile);
int firm_demangle (char *buff, char *buff2, size_t firm_start,
		   size_t firm_stop, size_t firm_end);
